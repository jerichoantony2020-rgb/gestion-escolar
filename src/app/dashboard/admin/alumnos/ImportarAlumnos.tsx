      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-2xl rounded-2xl border p-6 shadow-xl max-h-[90vh] overflow-y-auto" style={{ background: "var(--bg)", borderColor: "var(--border)" }}>
            {/* Acciones arriba: la previsualización lista decenas de alumnos y
                el botón de importar quedaba fuera de vista al final. */}
            <div className="sticky -top-6 z-10 flex items-start justify-between gap-3 -mx-6 px-6 pt-6 pb-3 -mt-6" style={{ background: "var(--bg)" }}>
              <div className="min-w-0">
                <h2 className="text-lg font-bold" style={{ color: "var(--fg)" }}>Importar alumnos desde Excel</h2>
                <p className="text-sm" style={{ color: "var(--muted)" }}>Descarga la plantilla, complétala y súbela. La columna Grado debe coincidir (ej. &quot;1° Grado&quot;, sección &quot;A&quot;).</p>
              </div>
              <div className="flex gap-2 shrink-0">
                <button onClick={close} className="px-3 py-2 rounded-lg border text-sm" style={{ borderColor: "var(--border)", color: "var(--muted)" }}>
                  {result ? "Cerrar" : "Cancelar"}
                </button>
                {rows.length > 0 && !result && (
                  <button onClick={doImport} disabled={importing} className="px-4 py-2 rounded-lg bg-primary-500 text-white text-sm font-semibold hover:bg-primary-600 disabled:opacity-60 whitespace-nowrap">
